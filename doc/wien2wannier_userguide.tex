...
%%%
%%% $Id: wien2wannier_userguide.tex 436 2015-09-01 06:27:31Z assmann $
...
\newcommand*\tit{\texorpdfstring{\Wtow}{Wien2Wannier} 1.0.1 User's
  Guide}
...
direct lattice vector $\mb{R}$, are called Wannier functions. Due to
the additional phase factors $U^{(\mb{k})}_{nm}$, which all lead to
valid Wannier functions, there is considerable ambiguity in the choice
of the real space basis set.  This can be overcome by choosing Wannier
orbitals $w_{m\mb{R}}(\mb{r})$ with minimal real-space extent (spread
$\langle\Delta \mb{r}^2\rangle$).  These are known as maximally
localized Wannier functions~(\mlwf). A popular program to compute the
\mlwf for a given set of Bloch functions is
...
\begin{lines}
  \begin{flin}{mode, switch \rem{format(A3, A1)}}{}
  mode & $n$D& with $n=0,\ldots,3$, an $n$-dimensional grid will be
               specified\\
       & ANY & read arbitrary grid from \file{\case.grid}\\
  flag & O,N & when \code{mode = $n$D}: check for orthogonality or not\\
       & C,F & when \code{mode = ANY}: cartesian or fractional coordinates
  \end{flin}
%
  If \code{mode=$n$D} and \code{flag=O} or absent, grid axes will
  be checked for pairwise orthogonality; set \code{flag=N} for
  nonorthogonal axes. %
  If \code{mode=ANY} and \code{flag=C} or absent, the grid points
  in \file{\case.grid} are in cartesian coordinates (3 reals per
  line); if \code{flag=F}, they are in fractional coordinates (3
  numbers and divisor).
  ...
  If \code{mode=$n$D}, \textbf{l.~3} must be given $n$ times, once for
  each direction; if \code{mode=ANY}, it must be omitted.
  ...
  \begin{flin}[T]{iwan \rem{(free format)}}{Wannier function index}
...
\end{thebibliography}
\end{document}
In general, wien2wannier for spin-polarized cases will work best if the spin directions are “similar”, or if you need only one spin.  This is because you normally have only one case.inwf file.  If you need both spins and they are “really different” (i.e., require different case.inwf and possibly case.win files), you will have to do something like

$ for s in up dn; do
	x lapw1 -$s
	ln -sf case.inwf$s case.inwf
	ln -sf case.win$s case.win
	x w2w -$s
	x wannier90 -$s
done

where you have previously created suitable case.{inwf,win}{up,dn} files.

%%% Time-stamp: <2015-09-01 10:03:49 assman@faepop23.tu-graz.ac.at>
