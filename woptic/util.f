...
!------------------------------------------------------------------
  ...
  allocate(lattice%sym(lattice%nop,3,3),lattice%ksym(lattice%nop,3,3))
  do j=1,lattice%nop
      do i=1,3
  ...
  !read in from seed.outputkgen
     ...
     enddo
     read(unit_outputkgen,*)dummy
     do j=1,int(float(lattice%nop)/4.+.9)                                 
        read(unit_outputkgen,*)dummy
        i1=4*j-3                                                          
        i2=4*j-2                                                          
        i3=4*j-1                                                          
        ...
        !write(*,*)i1,i2,i3,i4                                                       
        do i=1,3                                                      
           read(unit_outputkgen,140) (lattice%ksym(i1,i,k),K=1,3),(lattice%ksym(i2,i,k),K=1,3), &
                                     (lattice%ksym(i3,i,k),K=1,3),(lattice%ksym(i4,i,k),K=1,3)                          
        enddo
      enddo 
!       do j=1,lattice%nop
!           lattice%ksym(j,1,:) = (/1d0,0d0,0d0/)
!           lattice%ksym(j,2,:) = (/0d0,1d0,0d0/)
!           lattice%ksym(j,3,:) = (/0d0,0d0,1d0/)
!       enddo
...
!--------------------------------------------------
   ...
   END FUNCTION line_count
    ...
    SUBROUTINE inverse3x3(A,AINV)
     ...
     FUNCTION round_real(inreal,ndigits) result(outreal)

      implicit none

      integer ndigits
      real*8 :: inreal,outreal,rounder
 
      rounder = 10d0**ndigits
      
      if (inreal.lt.0d0) then
          outreal =  dble(int(inreal * rounder - 0.5)) / rounder
      else
          outreal =  dble(int(inreal * rounder + 0.5)) / rounder
      endif
