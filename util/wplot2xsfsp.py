...
# by Nikolaus Frohner <e0526087@student.tuwien.ac.at>
# and Philipp Wissgott 10/01/10
...
def usage():
	sys.stderr.write('[I] transforms output of wplot into XCrySDen XSF Format()(spin-polarized version)\n')
	...
	sys.stderr.write('[I] -s or --noshift\tdo not shift Wannier function centres\n')
        sys.stderr.write('[I] --up or --dn specifies spin mode\n')
	sys.exit()
	
try:
	opts, args = getopt.getopt(sys.argv[1:], "hudnp" , ["help", "up" , "dn" , "noshift" , "nophase"])
...
psinkFile = seedName + '_'+str(wfIdx)+'.psink'+sp
psiargFile = seedName + '_'+str(wfIdx)+'.psiarg'+sp
wplotoutFile = seedName + '.wplotout'+sp
structFile = seedName + '.struct'
xyzFile = seedName + '_centres.xyz'+sp
woutFile = seedName + '.wout'+sp
atomMap = {}
centers = []
