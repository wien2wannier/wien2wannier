PROGRAM find_bands
 ...
 if(iarg.eq.3) then
     ...
     write(*,*) 'Error: 3 commandline arguments have to be given.'
     stop
  endif
  ...
  !get Fermi energy
  open(unit=unitscf2,file=clearspace(seedname)//'.scf2',status='old')
  nlines=line_count(clearspace(seedname)//'.scf2',unitscf2)
  ...
  if (Ffound_Fermienergy.eqv. .false.) then
    write(*,*)"Did not find Fermi energy in "//clearspace(seedname)//'.scf2'
    stop
  else
     write(*,*)"Fermi energy(from "//clearspace(seedname)//".scf2 file) is: ",sngl(Fermienergy),"Ry=",sngl(Fermienergy*13.6),"eV"
  endif
  

  !open seed.output1
  open(unit=unitout1,file=clearspace(seedname)//'.output1',status='old')
  nlines=line_count(clearspace(seedname)//'.output1',unitout1)
  counter = 0
  kidx = 0
  kread =.false.
  !write(*,*)counter,nlines
